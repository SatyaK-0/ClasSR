...
class SymanticModel:
  ...
  def __init__(self,df,operators=None,multi_task = None,n_expansion=None,n_term=None,sis_features=20,device='cpu',relational_units = None,initial_screening = None,dimensionality=None,output_dim = None,metrics=[0.06,0.995],disp=False,pareto=False, A=None, B = None):
        ...
        if self.multi_task!=None:
            ...
            for i in range(len(self.multi_task_target)):
                ...
                if self.no_of_operators==None:
                    ...
                    rmse,equation,r2,_ = fcc.feature_space_construction(self.operators,df1,self.no_of_operators,self.device,self.initial_screening,self.metrics,dimension=self.dimension,sis_features=self.sis_features,disp=self.disp,pareto=self.pareto, A= self.A, B=self.B).feature_space()
                    ...
                    x,y,names,complexity = fcc.feature_space_construction(self.operators,df1,self.no_of_operators,self.device,self.initial_screening,disp=self.disp,pareto=self.pareto, A= self.A, B=self.B).feature_space()
                    
                    from .Regressor import Regressor
                    
                    rmse, equation,r2,r,c,n,intercepts,coeffs,_ =  Regressor(x,y,names,complexity,self.dimension,self.sis_features,self.device, A=self.A, B = self.B).regressor_fit()
            ...
            rmse,equation,r2,final = fcc.feature_space_construction(self.operators,self.df,self.no_of_operators,self.device,self.initial_screening,self.metrics,dimension=self.dimension,sis_features=self.sis_features,disp=self.disp,pareto=self.pareto, A= self.A, B=self.B).feature_space()
            ...
            x,y,names,complexity = fcc.feature_space_construction(self.operators,self.df,self.no_of_operators,self.device,self.initial_screening,disp=self.disp, A= self.A, B=self.B).feature_space()
